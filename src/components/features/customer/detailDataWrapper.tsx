...
export default function DetailDataWrapper({ initialCustomer, initData, }: Props) {
   ...
   const [accountData, setAccountData] = useState(initialCustomer.account);
   const [savingPeriod, setSavingPeriod] = useState(initialCustomer.account.savingPeriod);
   ...
   const getSumOfTransactions = (transactions: any[]) => {
      return transactions.reduce((sum, transaction) => sum + (transaction.points > 0 ? transaction.points : 0), 0);
   }
   ...
   const getSumOfTransactionsInYear = (transactions: any[], year: number) => {
      return transactions.reduce((sum, transaction) => sum + (transaction.points > 0 && transaction.year === year ? transaction.points : 0), 0);
   }
   ...
   return (
      <div className="h-full w-full flex flex-col gap-6 px-8 rounded-lg">
