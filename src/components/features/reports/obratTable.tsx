...
export default function ReportObratTable({ defaultData, detailLinkPath, updatedTime }: Props) {
    // Change registrationNumber to string
    defaultData.forEach(row => {
        row.registrationNumber = row.registrationNumber.toString()
        if (typeof row.salesManager !== 'string')
            row.salesManager = row.salesManager ? String(row.salesManager) : ''
    })
        ...
        () => {
            ...
            const baseColumns: ColumnDef<any>[] = [
                ...
                },
                {
                    ...
                    cell: ({ row }) => (
                        <Link
                            href={`/sales-managers/${row.original.salesManagerId}/stats`}
                            className="text-blue-600 hover:text-blue-800 hover:underline">
