...
import {
    useSavingPeriodByAccount,
    useTransactionsBySavingPeriodId,
} from '@/lib/queries/savingPeriod/queries'
...
export default function SavingPeriodStats({ account_id }: Props) {
    ...
    if (isSavingPeriodLoading || isTransactionsLoading)
        return <Skeleton className="w-2/4" type="chart" />
    ...
    return (
        ...
        //   />
        // </Card>
        // <Card className="p-6 flex flex-col gap-6 bg-white shadow-md rounded-xl w-full max-w-4xl">
        //     <KpiCardProgress
        //         title={`Dvouleté šetřící období od ${savingPeriod?.startYear}-Q${savingPeriod?.startQuarter} do ${savingPeriod?.endYear}-Q${savingPeriod?.endQuarter}`}
        //         points={savingPeriod?.availablePoints}
...
function prepareSavingPeriodChartData(
    ...
    transactions.forEach(transaction => {
        const quarterIndex = quarters.findIndex(
            q => q.quarter === `${transaction.year}-Q${transaction.quarter}`,
        )
