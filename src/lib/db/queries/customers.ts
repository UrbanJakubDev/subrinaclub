...
export async function getCustomers(active: boolean = true): Promise<Customer[]> {
    ...
    //     customer.totalPoints = customer.account.reduce((total, account) => {
    ...
    // customers.forEach(customer => {
    //     customer.currentYearPoints = customer.account.reduce((total, account) => {
    //         const sum = account.transactions.reduce((total, transaction) => {
    ...
    // customers.forEach(customer => {
    //     customer.hasCurrentYearPoints = customer.account.some(account =>
    ...
    // customers.forEach(customer => {
    //     customer.lastTransactionYear = customer.account
    ...
    // customers.forEach(customer => {
    //     customer.pointsInActiveSavingPeriod = customer.account
    //         .flatMap(account => {
...
export async function insertCustomerIntoDB(customer: Customer): Promise<Customer> {
...
export async function updateCustomerInDB(id: number, customer: Customer): Promise<Customer> {
    // Exclude the `accounts` from the data object to be updated
    const { account, id:_, dealerId, salesManagerId, ...customerData } = customer;

    try {
        // Create the data object without 'accounts' field
        const data: any = {
            ...customerData,
        };
        ...
        const updatedCustomer = await prisma.customer.update({
            ...
            data: data, // This 'data' object does not include 'accounts'
        });
...
export async function restoreCustomerById(id: number): Promise<Customer> {
...
export async function geetCustomersForReportSeznamObratu() {
...
export async function getMaxRegistrationNumber(): Promise<number> {
...
export async function findCustomerByIcoOrFullName(ico: string): Promise<Customer> {
